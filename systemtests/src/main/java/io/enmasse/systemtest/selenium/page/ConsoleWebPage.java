...
public class ConsoleWebPage implements IWebPage {
    ...
    public AddressSpaceConsoleWebPage openAddressSpaceConsolePage(AddressSpace addressSpace) throws Exception {
        AddressSpaceWebItem item = selenium.waitUntilItemPresent(30, () -> getAddressSpaceItem(addressSpace));
        selenium.clickOnItem(item.getConsoleRoute());
        AddressSpaceConsoleWebPage consolePage = new AddressSpaceConsoleWebPage(selenium, addressSpace, credentials);
        consolePage.login();
        return consolePage;
    }
